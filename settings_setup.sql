-- SQL skript pre vytvorenie tabuľky nastavení webu a vloženie predvolených hodnôt.
-- Tento skript spustite v Supabase SQL Editore.

-- 1. Vytvorenie tabuľky pre nastavenia webu
...
-- 3. RLS Politika pre čítanie (ktokoľvek môže čítať nastavenia pre zobrazenie na stránke)
drop policy if exists "Povoliť verejné čítanie nastavení" on site_settings;
create policy "Povoliť verejné čítanie nastavení"
on site_settings for select
using ( true );

-- 4. RLS Politika pre zápis/úpravu/mazanie (ktokoľvek pre zjednodušenie, respektíve admin)
drop policy if exists "Povoliť verejnú úpravu nastavení" on site_settings;
create policy "Povoliť verejnú úpravu nastavení"
on site_settings for all
using ( true )
with check ( true );

-- 5. Vloženie predvolených hodnôt (ak ešte neexistujú)
insert into site_settings (key, value) values
('hours_weekday', '07:00 - 12:00, 12:30 - 16:30') on conflict (key) do nothing;

insert into site_settings (key, value) values
('hours_saturday', '07:00 - 11:00') on conflict (key) do nothing;
...
insert into site_settings (key, value) values
('contact_email', 'kubik@stavivalubela.sk') on conflict (key) do nothing;

insert into site_settings (key, value) values
('billing_name', 'STAVIVÁLUBELA S.R.O.') on conflict (key) do nothing;

insert into site_settings (key, value) values
('billing_ico', '52 079 716') on conflict (key) do nothing;

insert into site_settings (key, value) values
('billing_icdph', 'SK2120884161') on conflict (key) do nothing;
